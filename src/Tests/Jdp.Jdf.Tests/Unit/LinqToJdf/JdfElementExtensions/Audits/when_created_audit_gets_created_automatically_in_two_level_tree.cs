...
namespace Jdp.Jdf.Tests.Unit.LinqToJdf.JdfElementExtensions.Audits {
    ...
    public class when_created_audit_gets_created_automatically_in_two_level_tree {
        ...
        It should_have_agent_name_from_configuration = () => document.Root.AuditPoolElement().Element(Audit.Created).GetAttributeValueOrNull("AgentName").ShouldEqual(JdpLibrary.Settings.AgentName);

        It should_have_agent_version_from_configuration = () => document.Root.AuditPoolElement().Element(Audit.Created).GetAttributeValueOrNull("AgentVersion").ShouldEqual(JdpLibrary.Settings.AgentVersion);

        It should_have_author_from_configuration = () => document.Root.AuditPoolElement().Element(Audit.Created).GetAttributeValueOrNull("Author").ShouldEqual(JdpLibrary.Settings.Author);

        It should_have_a_timestamp_ending_with_z_since_it_was_utc = () => document.Root.AuditPoolElement().Element(Audit.Created).GetAttributeValueOrNull("TimeStamp").ShouldEndWith("Z");
    }
}