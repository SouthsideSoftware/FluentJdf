...
namespace Jdp.Jdf.Tests.Unit.LinqToJdf.ResourceExtensions
{
    ...
    public class when_using_get_resource_or_null_with_resource_in_local_resource_pool {
        ...
        Establish context = () => ticket = Ticket.Create().AddIntentNode().AddInput(Resource.BindingIntent, "bi").AddOutput(Resource.Component, "c").Document;

        It should_be_able_to_find_the_binding_intent = () => ticket.Root.GetResourceOrNull("bi").ShouldEqual(ticket.Root.ResourcePool().Element(Resource.BindingIntent));

        It should_be_able_to_find_the_component =
            () => ticket.Root.GetResourceOrNull("c").ShouldEqual(ticket.Root.ResourcePool().Element(Resource.Component));
