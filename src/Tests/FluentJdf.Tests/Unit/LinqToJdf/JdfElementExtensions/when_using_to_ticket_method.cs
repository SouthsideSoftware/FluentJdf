...
namespace FluentJdf.Tests.Unit.LinqToJdf.JdfElementExtensions
{
    ...
    {
        It should_be_able_to_use_on_empty_document = () => new XDocument().ToTicket().ShouldBe(typeof(FluentJdf.LinqToJdf.Ticket));

        It should_be_able_to_use_on_jdf_tree = () => new XDocument(new XElement(Element.JDF, new XElement(Element.JDF))).ToTicket().ShouldBe(typeof(FluentJdf.LinqToJdf.Ticket));
