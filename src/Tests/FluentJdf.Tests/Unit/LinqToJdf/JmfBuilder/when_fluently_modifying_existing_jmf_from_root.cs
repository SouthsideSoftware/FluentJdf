...
namespace FluentJdf.Tests.Unit.LinqToJdf.JmfBuilder {
    ...
    public class when_fluently_modifying_existing_jmf_from_root {
        static Message message;

        Establish context = () => message = Message.Create().AddCommand().SubmitQueueEntry().Message;

        Because of = () => message.Root.ModifyJmfNode().AddCommand().SubmitQueueEntry();
        ...
        It should_have_type_submit_queue_entry_for_first_command = () => message.Root.Elements(Element.Command).First().GetTypeAttribute().ShouldEqual(Command.SubmitQueueEntry);

        It should_have_type_submit_queue_entry_for_second_command = () => message.Root.Elements(Element.Command).Skip(1).Take(1).First().GetTypeAttribute().ShouldEqual(Command.SubmitQueueEntry);
