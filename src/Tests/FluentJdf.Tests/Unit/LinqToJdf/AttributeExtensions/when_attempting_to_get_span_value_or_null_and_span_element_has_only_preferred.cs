...
namespace FluentJdf.Tests.Unit.LinqToJdf.AttributeExtensions {
    ...
    public class when_attempting_to_get_span_value_or_null_and_span_element_has_only_preferred {
        static FluentJdf.LinqToJdf.Ticket ticket;
        static string spanValue;

        Establish context = () => ticket = FluentJdf.LinqToJdf.Ticket.CreateIntent().WithInput().BindingIntent().AddNode(Element.BindingType).With().Attribute("Preferred", "test2").Ticket;
        ...
        It should_have_value_from_preferred = () => spanValue.ShouldEqual("test2");
    }
}