...
namespace FluentJdf.Tests.Unit.Encoding.PassThroughEncoding {
    ...
    public class when_encoding_generic_xml {
        static EncodingResult result;
        static FluentJdf.Encoding.XmlTransmissionPart transmissionPart;

        Establish context = () => transmissionPart = new FluentJdf.Encoding.XmlTransmissionPart(new XDocument(new XElement("foo")), "test");
        ...
        It should_have_content_type_for_xml = () => result.ContentType.ShouldEqual(MimeTypeHelper.XmlMimeType);
