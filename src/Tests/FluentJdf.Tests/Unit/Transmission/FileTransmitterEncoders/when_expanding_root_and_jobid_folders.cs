...
namespace FluentJdf.Tests.Unit.Transmission.FileTransmitterEncoders {
    ...
    public class when_expanding_root_and_jobid_folders {
        ...
        Establish context = () => {
            ...
            jobId = 12.ToString();
            jobKey = 24.ToString();
            expanded = encoder.ExpandFolder(@"${Root}\${JobId}", guid, jobId.ToString(), jobKey);
        };

        It should_equal_temp_and_jobid_off_root = () => expanded.ShouldEqual(Path.Combine(@"c:\temp", jobId));
    }
}
