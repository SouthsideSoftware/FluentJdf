...
namespace FluentJdf.Tests.Unit.TemplateEngine.Template {
    ...
    public class when_generating_message_from_template_with_global_custom_formula_taking_five_parameters {
        ...
        Establish context = () => {
            messageTemplate = FluentJdf.LinqToJdf.Message.Create().AddCommand().SubmitQueueEntry().With()
                .Attribute("test", "[:test=customTestFunction(testParm1, testParm2, testParm3, testParm4, testParm5):]").Message;

            FluentJdf.Configuration.FluentJdfLibrary.Settings.WithTemplateEngineSettings()
                .CustomFormula("customTestFunction", (parm1, parm2, parm3, parm4, parm5) => string.Format("this is a test {0}, {1}, {2}, {3}, {4}", parm1, parm2, parm3, parm4, parm5));
        };
